...
%     clear
    ...
    % pause
    model = @(t,x) ...
    [-0.4* x(1) * x(1) *((0.0003 * x(2) * x(2) + 0.008) / (0.2* x(1) * x(1) + 2.00001)) + 0.012 + (0.0000003 * (54660 - 5000.006* x(1)) * (0.2* x(1) * x(1) + 2.00001)) / (0.00036* x(2) * x(2) + 0.00960018 + 0.000000018* x(1) * x(1)); ...
    -0.0006* x(2) * x(2) + (0.000000006*  x(2) * x(2) + 0.00000016) / (0.2*  x(1) * x(1) + 2.00001) + (0.0015015 * x(1) *(0.2*  x(1) * x(1) + 2.00001)) / (0.00036 * x(2) * x(2) + 0.00960018 + 0.000000018* x(1) * x(1))];

    init_cond = [0.5 1.5; 0.0 1.5];
    input_range = [];
    cp_array = [];
